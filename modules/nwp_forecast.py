...
def fetch_gfs_pressure_level(lat, lon, hours=72):
    """获取 GFS 气压层垂直廓线数据（用于 Skew-T 探空图）。

    返回 (profile_dict, error_msg)。
    profile_dict = {level_hPa: {temp, dewpoint, wind_speed, wind_dir}, ...}
    """
    url = "https://api.open-meteo.com/v1/forecast"
    params = {
        ...
        "hourly": [
            ...
            "temperature_200hPa", "temperature_100hPa",
            ...
            "wind_direction_200hPa", "wind_direction_100hPa",
        ],
    ...
    try:
        ...
        return None, f"气压层请求失败: {e}"

    if "hourly" not in data:
        return None, "气压层 API 返回异常"

    p_levels = [1000, 925, 850, 700, 500, 300, 200, 100]
    h = data["hourly"]
    timestamps = h["time"]
    ...
    for i, ts in enumerate(timestamps):
        profile = {}
        for p in p_levels:
            t_key = f"temperature_{p}hPa"
            rh_key = f"relative_humidity_{p}hPa"
            ws_key = f"wind_speed_{p}hPa"
            wd_key = f"wind_direction_{p}hPa"

            temp = h.get(t_key, [None] * len(timestamps))[i]
            rh = h.get(rh_key, [None] * len(timestamps))[i]
            ws = h.get(ws_key, [None] * len(timestamps))[i]
            wd = h.get(wd_key, [None] * len(timestamps))[i]

            if temp is not None:
                # 露点温度近似：Td = T - ((100 - RH) / 5)
                rh_val = rh if rh is not None else 50
                td = temp - ((100 - rh_val) / 5)
                profile[p] = {
                    "temp": temp, "dewpoint": td,
                    "wind_speed": ws if ws is not None else 0,
                    "wind_dir": wd if wd is not None else 0,
                }
        profiles_by_time[ts] = profile

    return profiles_by_time, None


def _render_skewt(profile, title=""):
    """使用 MetPy 绘制 Skew-T 探空图，返回 Plotly Figure"""
    try:
        ...
        return None, "MetPy 未安装。请运行: pip install metpy"

    p_hpa = sorted(profile.keys(), reverse=True)
    temps = np.array([profile[p]["temp"] for p in p_hpa])
    dews = np.array([profile[p]["dewpoint"] for p in p_hpa])
    speeds = np.array([profile[p]["wind_speed"] for p in p_hpa])
    dirs = np.array([profile[p]["wind_dir"] for p in p_hpa])
    pressures = np.array(p_hpa)

    fig = plt.figure(figsize=(7, 7))
    skew = SkewT(fig, rotation=45)
    skew.plot(pressures, temps, "r", linewidth=2, label="温度")
    skew.plot(pressures, dews, "g", linewidth=2, label="露点")

    mask = pressures >= 100
    u = speeds[mask]
    v = np.zeros_like(u)
    skew.plot_barbs(
        pressures[mask][::len(pressures[mask]) // 8 + 1],
        u[::len(u) // 8 + 1],
        v[::len(v) // 8 + 1],
    )
    ...
    return f'<img src="data:image/png;base64,{img_b64}" style="width:100%;max-width:600px;">', None


def render_skewt_tab(lat, lon):
    """渲染 Skew-T 探空图子页面"""
    import streamlit as st
    st.write("### [探空] Skew-T 温度对数压力图")

    hours = st.slider("预报时次", 0, 72, 0, step=3, key="skewt_hours",
                      help="0=当前分析场, 3/6/...=未来预报时次")
    if st.button("[探空] 获取并绘制探空图", use_container_width=True, key="skewt_fetch"):
        with st.spinner("正在获取 GFS 气压层数据..."):
            profiles, err = fetch_gfs_pressure_level(lat, lon, hours=hours + 6)
        if err:
            st.error(err)
        elif not profiles:
            st.warning("未获取到气压层数据")
        else:
            times = sorted(profiles.keys())
            idx = min(hours // 3, len(times) - 1) if times else 0
            ts = times[idx]
            profile = profiles[ts]
            html, sk_err = _render_skewt(profile, title=f"Skew-T @ {lat:.2f}N, {lon:.2f}E | {ts}")
            if sk_err:
                st.error(sk_err)
            else:
                st.markdown(html, unsafe_allow_html=True)
                st.caption("红色: 温度廓线 | 绿色: 露点廓线 | 斜虚线: 干绝热线 | 斜实线: 湿绝热线")
