...
- 断点表的坐标轴约定：**浓度轴**按整数递进（相邻档 `lo == 上一档 hi + 1`）；**IAQI 轴连续**，相邻档共用同一节点（上一档 `iaqi_hi == 本档 iaqi_lo`），节点取自 `{0, 50, 100, 150, 200, 300, 400, 500}`。两轴都做 +1 递进是原项目的写法，属建模瑕疵，不得沿用。
...
> **实施记录（2026-09-13）**：首版计划把原项目「两个坐标轴都 +1 递进」的习惯一并沿用，写成 `(36, 75, 51, 100)`，被 `test_config_table_passes_validation` 当场拦下——51 不在标准 IAQI 节点内。国标只在浓度轴上按整数递进，IAQI 轴连续。上方代码块已是修正后的版本，并已在实施中通过验证。
