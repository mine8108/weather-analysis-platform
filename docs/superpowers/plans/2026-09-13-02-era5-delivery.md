...
## 实施记录补记（2026-09-13，v2.3.2）：变量表与线上 CDS 的真实核对

本批次的变量表取自 2026-09-13 的 CDS 实测快照，但**当时的证据全是本地一致性**——测试锁的是「catalogue 与快照一致」，快照本身是否仍与线上一致无人核对。上游增删或改名变量时，这类测试会一路保持绿色。

补做真实核对后确认 CDS 有一个**不需要账号与 API Key** 的公开接口：`https://cds.climate.copernicus.eu/api/retrieve/v1/processes/<数据集>`，其 `inputs.variable.schema.items.enum` 就是权威变量枚举。

核对结果（2026-09-13）：

| 产品 | 本地展示 | CDS 枚举 | 本地名字不存在 | 不支持声明误标 | CDS 未收录 |
|---|---|---|---|---|---|
| ERA5-Land | 12 | 60 | 0 | 0 | 48 |
| ERA5 单层 | 12 | 262 | 0 | 0 | 250 |
| ERA5 气压层 | 10 | 16 | 0 | 0 | 6 |
| ERA5-Land 月均值 | 12 | 60 | 0 | 0 | 48 |

反方向也核对了：本地标注为「不支持」的 6 项（如 ERA5-Land 的 `relative_humidity`、`total_cloud_cover`）确实都不在对应枚举中，没有把可用变量误判为不可用。

该核对已固化为 `research/check_era5_variables.py`（退出码 0 一致 / 1 不一致 / 2 联网失败，2 不等于通过），把原本「每季度人工重采快照」的维护动作降为一条命令。脚本不进 CI：上游故障不应让流水线变红。
