...
> 后续补充：v2.3.2 时发布号再由 2.3.1 升至 2.3.2，由 `.github/workflows/tests.yml`、`.streamlit/secrets.toml.example`、`README.md` 与 `config.py` 四处同步。上面关于号段取舍的判断不变。

---

## 附录 D · 图片编码设计的更正（2026-09-13，v2.3.2）

本设计 §5.2 的常量表与处理步骤规定「统一转 RGB，以 JPEG 质量 88 编码一次」，并把 `MAX_IMAGE_BYTES` / `MAX_TOTAL_BYTES` 的判定对象写为「压缩后字节」。**这条设计是错的**，已由真实天气图推翻，详见 `docs/superpowers/plans/2026-09-13-04-chart-reader.md` 末尾的「实施记录补记（v2.3.2）」。

结论摘要：

- 天气图是线画类内容，JPEG 对其效率极差。实测 JMA 600×512 PNG 64 KB → JPEG 85 KB（+33%），WPC 748×562 GIF 35 KB → JPEG 181 KB（5.2 倍）。
- 现行实现改为「能不改就不改」：未缩放且为 PNG/JPEG 时原样提交；需重编码时在 JPEG 与 PNG 中取更小者，并只用这两种格式（python-docx 不支持 WebP）。
- 判定对象与界面措辞由「压缩后」改为「提交」。
- 接口变化：`process_image` 返回 `data_bytes` / `mime` / `kept_original` 取代 `jpeg_bytes`；`image_to_data_url(data, mime)` 增加第二参数；会话结构 `chart_reader_images` 的键名随之改变。

本设计正文的上述段落保留原样作为决策轨迹，不回溯改写；凡与实现冲突，以实现与本附录为准。
